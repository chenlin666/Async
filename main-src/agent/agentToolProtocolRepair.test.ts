...
describe('stripOrphanToolResultsFromAssistantContent (extended tool_call attrs)', () => {
	it('keeps paired tool_call with sub_parent/sub_depth and tool_result', () => {
		const content = [
			'\n<tool_call tool="read_file" sub_parent="call-1" sub_depth="1">{"path":"a.ts"}</tool_call>\n',
			'<tool_result tool="read_file" success="true">1|ok</tool_result>\n',
		].join('');
	...
	it('removes orphan tool_result when no preceding call', () => {
		const content = 'intro\n<tool_result tool="read_file" success="true">orphan</tool_result>\ntrailing';
